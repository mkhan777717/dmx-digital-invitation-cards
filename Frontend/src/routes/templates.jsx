      ...
      <div className="space-y-10 animate-reveal-up">
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between border-b border-white/5 pb-8">
          <div>
            ...
            >
              <Typewriter text="Bespoke Art" speed={35} /><em className="text-gold-gradient not-italic"> Library</em>
            </h1>
          ...
          {/* Search bar */}
          <div className="animate-slide-right relative w-full max-w-xs">
        ...
        <div className="flex flex-wrap gap-2.5">
          {categories.map((cat) => (
            <button
              ...
              className={`animate-fade-up rounded-full px-5 py-2 text-xs uppercase tracking-wider transition duration-500 hover:-translate-y-0.5 ${categoryFilter === cat
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {filteredTemplates.map((t, i) => (
              <article
                key={t._id}
                className="group relative flex flex-col justify-between overflow-hidden rounded-3xl glass transition-all duration-700 animate-fade-up hover:-translate-y-2 hover:shadow-[0_30px_80px_-20px_rgba(0,0,0,0.65)]"
                style={{
              ...
              >
                ...
                <div className="relative aspect-[4/5] overflow-hidden bg-noir/40">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 flex flex-col justify-end">
                    ...
                    <h3 className="animate-slide-left font-display text-2xl text-foreground mt-1.5">
                      {t.title}
                    </h3>
                    ...
                    <p className="animate-slide-right text-xs text-white/60 line-clamp-2 max-h-0 opacity-0 group-hover:max-h-12 group-hover:opacity-100 transition-all duration-500">
                ...
                <div className="flex items-center justify-between border-t border-white/5 p-6 bg-noir/20">
                  <div className="flex flex-col">
                    ...
                    <span className="animate-slide-left text-gold-gradient font-display text-lg font-semibold floating-text">{t.price}</span>
                  </div>
                  <Link
                    to="/create"
                    search={{ templateId: t._id }}
                    className="animate-fade-up !py-2.5 !px-5 text-xs uppercase tracking-widest flex items-center gap-1 rounded-full bg-yellow-400 hover:bg-yellow-500 text-black transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(250,204,21,0.8)]"
                  >
                    Select Design
